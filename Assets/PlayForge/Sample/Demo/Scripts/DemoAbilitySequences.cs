...
namespace FarEmerald.PlayForge.Extended.Examples
{
    ...
    public static class DemoAbilitySequences
    {
            ...
            return TaskSequenceBuilder.Create("Fireball")
                ...
                .SyncTask((d, dt) =>
                {
                    ...
                    if (impactTimer >= punchDuration)
                    {
                        ...
                        var size = Physics.OverlapSphereNonAlloc(fireball.transform.position, 3f * punch, hits, DemoManager.Instance.PickerMask);
        ...
        [TaskSequenceMethod("Test Async Ability Sequence")]
        public static TaskSequence TestAsyncAbilitySequence()
        {
            return TaskSequenceBuilder.Create("Testing Async")
                .Do(d =>
                {
                    Debug.Log($"A");
                })
            ...
            return TaskSequenceBuilder.Create("Heal Pulse")
                ...
                .SyncTask((d, dt) =>
                {
                    ...
                    else if (timer < expandDuration + holdDuration)
                    {
                        ...
                        {
                            var hits = new Collider[16];
                            var size = Physics.OverlapSphereNonAlloc(origin, maxRadius, hits, DemoManager.Instance.PickerMask);
                            Debug.Log($"A");
                            var effects = d.GetLoadedAssets<GameplayEffect>(Tags.EFFECTS);
                            if (!effects.IsValid) Debug.Log($"No effects found :(");
                            else Debug.Log($"Heal pulse effect: {effects.Primary.GetName()}");
                            for (int i = 0; i < size; i++)
                            {
                                if (hits[i] is null) continue;
                                Debug.Log($"\t\t{i}: {hits[i].gameObject.name}");
                                d.TryApplyEffects(hits[i].gameObject, Tags.EFFECTS);
                            }
                            appliedEffect = true;
                        }
            ...
            return TaskSequenceBuilder.Create("Meteor Strike")
                ...
                .SyncTask((d, dt) =>
                {
                    ...
                    if (Vector3.Distance(meteor.transform.position, targetPos) < 1f)
                    {
                        ...
                        var hits = new Collider[16];
                        var size = Physics.OverlapSphereNonAlloc(targetPos, 4f, hits, DemoManager.Instance.PickerMask);
                        for (int i = 0; i < size; i++)
                        {
                            d.TryApplyEffects(hits[i].gameObject, Tags.EFFECTS);
                        }
            ...
            return TaskSequenceBuilder.Create("Dash Strike")
                ...
                .SyncTask((d, dt) =>
                {
                    ...
                    if (impactTimer >= punchDur)
                    {
                        ...
                        var hits = new Collider[16];
                        var size = Physics.OverlapSphereNonAlloc(targetPos, 1f, hits, DemoManager.Instance.PickerMask);
                        for (int i = 0; i < size; i++)
                        {
                            d.TryApplyEffects(hits[i].gameObject, Tags.EFFECTS);
                        }
                ...
                .SyncTask((d, dt) =>
                {
                    ...
                    for (int i = 0; i < orbs.Count; i++)
                    {
                        ...
                        if (t >= 1f)
                        {
                            ...
                            var hits = new Collider[16];
                            var size = Physics.OverlapSphereNonAlloc(dest, .5f, hits, DemoManager.Instance.PickerMask);
