...
{
    /// <summary>
    /// Custom property drawer for IAbilityValidationRule that extends AbstractTypeRefDrawer
    /// to also display the serialized fields of each concrete validation rule implementation.
    /// 
    /// Layout:
    /// [Type Name Button] [X] [>]
    /// [Child Fields Container]
    ///   ├── Field 1 (e.g., Tag)
    ///   ├── Field 2 (e.g., Attribute)
    ///   └── Field 3 (e.g., Comparison, Threshold)
    /// [Dropdown - when open]
    /// </summary>
    ...
    public class AbilityValidationRuleDrawer : AbstractTypeRefDrawer<IAbilityValidationRule>
    {
        ...
        {
            return typeof(IsAliveValidation);
        }
    }
}